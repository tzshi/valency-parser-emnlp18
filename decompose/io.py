...
from .algorithm import get_part_label, get_layer_labels


def read_conll(filename):
    def get_word(columns):
        return Word(columns[FORM], columns[UPOS], lemma=columns[LEMMA], xpos=columns[XPOS], feats=columns[FEATS], deps=columns[DEPS], misc=columns[MISC])
    ...
    def get_graph(graphs, words, tokens, edges, comments):
        ...
        for i in range(len(graph.nodes)):
            graph.nodes[i].part = get_part_label(graph, i)
            graph.nodes[i].layers = get_layer_labels(graph, i)
    ...
    while True:
        ...
        if "-" in columns[ID]:
            ...
            for _ in range(start, end + 1):
                ...
                #  edges.append((int(word_columns[HEAD]), int(word_columns[ID]), word_columns[DEPREL]))
        # Basic tokens/words
        else:
            ...
            #  edges.append((int(columns[HEAD]), int(columns[ID]), columns[DEPREL]))
